...
![案件一覧画面(絞り込み)](./images/案件一覧画面(絞り込み).png)
...
![案件一覧画面(新規)](./images/案件一覧画面(新規).png)

![案件情報(新規)](./images/案件情報(新規).png)

#### 編集

一覧で編集したい案件の「件名」をクリックすると、その案件の編集画面に移動します。

![案件一覧画面(件名)](./images/案件一覧画面(件名).png)

![案件情報(修正)](./images/案件情報(修正).png)
...
1.  **既存の取引先から選択**: 「相手先」の「検索キー」欄に取引先管理で設定した「呼び出しキー」を入力すると、呼び出しキーにマッチした会社が一覧で表示されます。ここから選択すると、その取引先の情報（会社名、住所、担当者名など）が自動で入力されます。

![既存の取引先を選択](./images/既存の取引先を選択.png)
...
![取引先を直接入力](./images/取引先を直接入力.png)

### その他の項目

*   **日付**: 発生日、納期、終了日といった日付を管理します。
*   **件名**: 案件のタイトルです。
*   **担当**: 社内の担当者を設定します([役職員管理](./member.md)で設定したものが選択に出ます)。
...
![取引文書作成ボタン](./images/取引文書作成ボタン.png)
...
![案件に紐づく取引一覧](./images/案件に紐づく取引一覧.png)
